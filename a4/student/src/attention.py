...
def precompute_rotary_emb(dim, max_positions):
    ...
    theta_i = torch.tensor(theta_i).unsqueeze(0)  # (1, dim / 2)
    angles = positions * theta_i  # (max_positions, dim / 2)
    cos_values = torch.cos(angles)  # (max_positions, dim / 2)
    sin_values = torch.sin(angles)  # (max_positions, dim / 2)
    rope_cache = torch.stack((cos_values, sin_values), dim=-1)  # (max_positions, dim / 2, 2)
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    rotated_x = None
    ### YOUR CODE HERE ###
    B, T, H, D = x.size() # (B, T, H, C/nh)
    rope = rope_cache[:T]  # (T, C/2, 2)
    rope = torch.view_as_complex(rope)  # (T, C/2)

    x_pairs = x.view(B, T, H, D // 2, 2)  # (B, T, H, C/nh, 2)
    x_complex = torch.view_as_complex(x_pairs)  # (B, T, H, C/nh, 2)

    rope = rope.view(1, T, 1, D // 2)  # (1, T, 1, D/2), 广播到(B, T, C/2)
    x_rot_complex = x_complex * rope  # (B, T, C/2)
    x_rot_pairs = torch.view_as_real(x_rot_complex)  # (B, T, C/2, 2)
    rotated_x = x_rot_pairs.view(B, T, H, D)
    ### END YOUR CODE ###
    return rotated_x
