...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(input_size=embed_size + hidden_size, hidden_size=hidden_size, bias=True)  # 单步解码
        self.h_projection = nn.Linear(in_features=2 * hidden_size, out_features=hidden_size, bias=False)  # W_h
        self.c_projection = nn.Linear(in_features=2 * hidden_size, out_features=hidden_size, bias=False)  # W_c
        self.att_projection = nn.Linear(in_features=2 * hidden_size, out_features=hidden_size, bias=False) # W_attProj
        self.combined_output_projection = nn.Linear(in_features= hidden_size + 2 * hidden_size, out_features=hidden_size, bias=False)  # W_u
        self.target_vocab_projection = nn.Linear(in_features=hidden_size, out_features=len(vocab.tgt), bias=False)  # W_vocab
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        # init_decoder_hidden
        last_hidden_cat = torch.cat((last_hidden[0], last_hidden[1]), dim=-1)  # (2, b, h) -> (b, 2*h)
        init_decoder_hidden = self.h_projection(last_hidden_cat)  # (b, h)
        # init_decoder_cell
        last_cell_cat = torch.cat((last_cell[0], last_cell[1]), dim=-1)  # (2, b, h) -> (b, 2*h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # torch.bmm() 按batch做矩阵乘法
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(-1)).squeeze(-1)  # (b, src_len, h) * (b, h, 1) -> (b, src_len, 1) -> (b, src_len)
        ### END YOUR CODE
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1) # (b, src_len) -> (b, 1, src_len) * (b, src_len, 2h) -> (b, 1, 2h) -> (b, 2h)
        U_t = torch.cat([dec_hidden, a_t], dim=-1)  # (b, h + 2h)
