...
parser = argparse.ArgumentParser(description='Train neural dependency parser in pytorch')  # argparse模块的标准用法，用来创建命令行参数解释器
parser.add_argument('-d', '--debug', action='store_true', help='whether to enter debug mode')
args = parser.parse_args() # 读取用户在命令行输入的参数 并封装成对象的形式返回
